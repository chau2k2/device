...
namespace device.Validation
{
    public class LaptopValidate : AbstractValidator<Laptop>
    {
        public LaptopValidate()
        {
            ...
            RuleFor(lap => lap.CostPrice).GreaterThan(0).WithMessage("Gia von phai lon hon 0");
            RuleFor(lap => lap.SalePrice).GreaterThan(lap => lap.CostPrice).WithMessage("gia ban phai lon hon gia nhap");
        }
    }
}
